...
git clone -c core.longpaths=true #clona um repositório e habilita caminhos longos
...
git pull --rebase origin master #atualiza o repositório local com o repositório remoto realizando o rebase (sem merge commit)
...
Remove-Item -Recurse -Force -Verbose -Path "nome da pasta" #versão para PowerShell
#usando git flow
